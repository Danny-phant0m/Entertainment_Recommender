    ...
    def _predict_all(self):
        ...
        if self.kind == "user":
            ...
            for user_id, k_users in enumerate(sorted_ids): # loop through each user
                # print(user_id)
                # print(k_users)
                # print(self.sim_m[user_id, k_users].flatten())
...
def objective(trial):
    ...
    model = Recommender(n_users, n_movies, train_set, kind="user", k=k, bias_sub=bias_sub)
...
print_recommendations(recommender2, movies, "Superman Returns")